...
export class SwitchToggler extends MiniGame {
  ...
  headerText = this.prompt.addText(
    'TURN EVERYTHING ' + (this.targetState ? 'ON' : 'OFF'),
    0,
    250,
    Color4.Green(),
    40
  ...
  SetupSwitches() {
    ...
    for (let row = 0; row < this.rows; row++) {
      for (let column = 0; column < this.columns; column++) {
        const newSwitch = this.prompt.addSwitch(
          '',
          startingPos.x + column * spaceBetweenSwitches,
          startingPos.y + row * spaceBetweenSwitches,
          () => {
